...
async def fetch_and_parse_feed(client, url):
    ...
    try:
        ...
        return feed_items
    except (httpx.HTTPError, ET.ParseError, ValueError, IndexError):
...
def register(mcp):
    ...
    async def get_world_finance_news() -> str:
        ...
        if not all_articles:
            return (
                "The financial feeds are unresponsive right now, sir. "
                "I can't pull market headlines."
            )
    ...
    async def open_world_monitor() -> str:
        ...
        try:
            ...
            return f"I'm unable to initialize the visual monitor: {e!s}"

    @mcp.tool()
    async def open_finance_world_monitor() -> str:
        """
        Opens the Finance World Monitor dashboard
        (finance.worldmonitor.app) in the system's web browser.
        Use this when the user wants a visual overview of global
        ...
        try:
            ...
            return f"I'm unable to initialize the finance monitor: {e!s}"
