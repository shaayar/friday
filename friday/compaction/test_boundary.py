...
def test_single_compaction_boundary() -> None:
    compaction = make_compaction(
        compaction_id="c1", first_message_id=1, last_message_id=20
    )
...
def test_no_boundary_selects_from_first_available_message() -> None:
    selected = select_compaction_window(
        messages(1, 2, 3, 4, 5), next_start=None, max_window=20
    )
...
def test_window_starts_at_boundary() -> None:
    selected = select_compaction_window(
        messages(1, 2, 3, 4, 5), next_start=3, max_window=20
    )
...
def test_messages_before_boundary_ignored() -> None:
    selected = select_compaction_window(
        messages(1, 2, 3, 4, 5), next_start=4, max_window=20
    )
...
def test_exactly_max_window_messages_all_selected() -> None:
    selected = select_compaction_window(
        messages(1, 2, 3), next_start=None, max_window=3
    )
...
def test_more_than_max_window_only_max_selected() -> None:
    selected = select_compaction_window(
        messages(1, 2, 3, 4, 5), next_start=None, max_window=3
    )
    assert [msg.id for msg in selected] == [1, 2, 3]
...
def test_fewer_than_max_window_all_remaining_selected() -> None:
    selected = select_compaction_window(
        messages(1, 2, 3), next_start=None, max_window=10
    )
    assert [msg.id for msg in selected] == [1, 2, 3]
...
def test_input_ordering_preserved() -> None:
    selected = select_compaction_window(
        messages(1, 2, 3, 4, 5), next_start=2, max_window=3
    )
...
def test_invalid_max_window_rejected(max_window) -> None:
    with pytest.raises((TypeError, ValueError)):
        select_compaction_window(
            messages(1, 2, 3), next_start=None, max_window=max_window
        )
...
def test_original_message_objects_returned() -> None:
    ...
    assert all(a is b for a, b in zip(selected, original, strict=False))
...
def test_boundary_with_non_contiguous_compactions() -> None:
    ...
    assert boundary == 71
    selected = select_compaction_window(
        messages(*range(1, 81)), next_start=boundary, max_window=20
    )
