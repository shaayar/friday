...
class TestSaveGet:
    def test_save_get_round_trip(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
    ...
    def test_all_four_categories_round_trip(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        compaction = all_categories_compaction(conversation_id)
        store.save(compaction)
        retrieved = store.get(compaction.compaction_id)
        assert retrieved is not None
        assert retrieved.summary == "Covered storage design."
        assert [i.content for i in retrieved.facts] == ["Storage uses conversations.db.", "Memory stays in memory.db."]
        assert [i.content for i in retrieved.decisions] == ["Use SQLite for compaction."]
    ...
    def test_multiple_items_per_category(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
    ...
    def test_multiple_provenance_ids(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        compaction = make_compaction(
            compaction_id="prov",
            conversation_id=conversation_id,
            first_message_id=1,
    ...
    def test_empty_categories_round_trip(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        compaction = make_compaction(
            compaction_id="empty",
            conversation_id=conversation_id,
            first_message_id=1,
    ...
    def test_storage_does_not_mutate_domain_object(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        compaction = all_categories_compaction(conversation_id)
...
class TestIdentityAndIdempotency:
    def test_duplicate_compaction_id_rejected(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        compaction = all_categories_compaction(conversation_id)
        store.save(compaction)
        with pytest.raises(CompactionAlreadyExistsError):
            store.save(compaction)

    def test_different_ranges_distinct_records(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        c1 = make_compaction(compaction_id="c1", conversation_id=conversation_id, first_message_id=1, last_message_id=2)
        c2 = make_compaction(compaction_id="c2", conversation_id=conversation_id, first_message_id=3, last_message_id=4)
        store.save(c1)
        store.save(c2)
...
class TestListing:
    def test_conversation_isolation(self, store: SQLiteCompactionStore, db_path: Path, conversation_id: int) -> None:
        ...
        store.save(make_compaction(compaction_id="a", conversation_id=1, first_message_id=1, last_message_id=4))
        store.save(make_compaction(compaction_id="b", conversation_id=conv_b.id, first_message_id=1, last_message_id=3))
    ...
    def test_multiple_compactions_one_conversation(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        store.save(make_compaction(compaction_id="c1", conversation_id=conversation_id, first_message_id=1, last_message_id=2))
        store.save(make_compaction(compaction_id="c2", conversation_id=conversation_id, first_message_id=3, last_message_id=4))
        ids = [c.compaction_id for c in store.list_for_conversation(conversation_id)]
        assert set(ids) == {"c1", "c2"}

    def test_list_for_conversation_ordering_deterministic(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        store.save(make_compaction(compaction_id="c1", conversation_id=conversation_id, first_message_id=1, last_message_id=2))
        store.save(make_compaction(compaction_id="c2", conversation_id=conversation_id, first_message_id=3, last_message_id=4))
        ids = [c.compaction_id for c in store.list_for_conversation(conversation_id)]
        assert ids == ["c1", "c2"]

    def test_get_latest_by_last_message_id(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        store.save(make_compaction(compaction_id="c1", conversation_id=conversation_id, first_message_id=1, last_message_id=2))
        store.save(make_compaction(compaction_id="c2", conversation_id=conversation_id, first_message_id=3, last_message_id=4))
    ...
    def test_get_latest_none_when_no_compactions(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
...
class TestDurability:
    def test_survives_close_reopen(self, db_path: Path, conversation_id: int) -> None:
        ...
        with SQLiteCompactionStore(db_path) as reopened:
            ...
            assert retrieved is not None
            assert retrieved == compaction
            assert [c.compaction_id for c in reopened.list_for_conversation(conversation_id)] == ["comp-1"]


class TestForeignKeys:
    def test_conversation_cascade_deletes_compactions(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        store.save(make_compaction(compaction_id="c1", conversation_id=conversation_id, first_message_id=1, last_message_id=4))
        with store._conn:
            store._conn.execute("DELETE FROM conversations WHERE id = ?", (conversation_id,))
    ...
    def test_compaction_delete_cascades_to_items_and_provenance(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        store.save(all_categories_compaction(conversation_id))
        with store._conn:
            store._conn.execute("DELETE FROM conversation_compactions WHERE compaction_id = ?", ("comp-1",))
        item_count = store._conn.execute(
            "SELECT COUNT(*) FROM compaction_items WHERE compaction_id = ?", ("comp-1",)
        ).fetchone()[0]
        assert item_count == 0
        prov_count = store._conn.execute("SELECT COUNT(*) FROM compaction_provenance").fetchone()[0]
        assert prov_count == 0

    def test_save_for_missing_conversation_fails(self, store: SQLiteCompactionStore) -> None:
        compaction = make_compaction(compaction_id="orphan", conversation_id=99999, first_message_id=1, last_message_id=4)
        with pytest.raises(CompactionCorruptError):
            store.save(compaction)

    def test_missing_source_message_rejected(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        compaction = make_compaction(
    ...
    def test_atomic_failure_no_partial_compaction(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        bad = make_compaction(
            compaction_id="atomic-bad",
            conversation_id=conversation_id,
            first_message_id=1,
            last_message_id=6,
            facts=(make_item("f1", "Fact.", (1, 2)), make_item("f2", "Ghost fact.", (5,))),
        )
        with pytest.raises(CompactionCorruptError):
            store.save(bad)
        assert store.get("atomic-bad") is None
        row = store._conn.execute(
            "SELECT COUNT(*) FROM compaction_items WHERE compaction_id = ?", ("atomic-bad",)
        ).fetchone()
        assert row[0] == 0


class TestValidation:
    def test_invalid_category_rejected(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        with pytest.raises(CompactionCorruptError, match="category"):
            store._validate_category("gossip")

    def test_unsupported_version_rejected_on_read(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        compaction = make_compaction(
            compaction_id="v2",
            conversation_id=conversation_id,
            first_message_id=1,
...
class TestIsolation:
    def test_no_raw_message_content_duplicated(self, store: SQLiteCompactionStore, conversation_id: int) -> None:
        store.save(all_categories_compaction(conversation_id))
        content_rows = store._conn.execute("SELECT content FROM compaction_items").fetchall()
        compaction_contents = {row[0] for row in content_rows}
        for i in range(1, 5):
            assert f"Message {i}" not in compaction_contents

    def test_no_memory_db_interaction(self, db_path: Path, tmp_path: Path, monkeypatch) -> None:
