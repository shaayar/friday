...
class TestExtraction:
    ...
    def test_valid_candidates_mapped(self) -> None:
    ...
    def test_project_fact_with_project_id(self) -> None:
        llm = FakeLLM(VALID_JSON)
        extractor = self._extractor(llm)
        candidates = extractor.extract(
    ...
    def test_project_fact_without_project_id_skipped(self) -> None:
        llm = FakeLLM(VALID_JSON)
        extractor = self._extractor(llm)
        candidates = extractor.extract(
            messages_from(message("m1", "user", "The project uses Next.js.")),
    ...
    def test_window_respects_most_recent_only(self) -> None:
        llm = FakeLLM(VALID_JSON)
        ...
        extractor.extract(messages, conversation_id="conv-1")
    ...
    def test_chronological_order_in_transcript(self) -> None:
        llm = FakeLLM(VALID_JSON)
        extractor = self._extractor(llm, window_size=20)
        extractor.extract(
            messages_from(
    ...
    def test_min_messages_not_met_skips_llm(self) -> None:
        llm = FakeLLM(VALID_JSON)
    ...
    def test_empty_messages_returns_empty(self) -> None:
        llm = FakeLLM(VALID_JSON)
        extractor = self._extractor(llm)
        assert extractor.extract([], conversation_id="conv-1") == []
        assert llm.calls == 0

    def test_llm_failure_is_isolated(self) -> None:
...
class TestParsing:
    ...
    def test_top_level_object_with_facts_key(self) -> None:
    ...
    def test_trivial_content_filtered(self) -> None:
        llm = FakeLLM(
    ...
    def test_non_object_items_skipped(self) -> None:
    ...
    def test_llm_supplied_message_ids_filtered_to_known(self) -> None:
        llm = FakeLLM(
            '[{"content": "User likes tea.", "type": "user_fact",'
    ...
    def test_inferred_with_insufficient_messages_skipped(self) -> None:
    ...
    def test_conversation_summary_scope(self) -> None:
        llm = FakeLLM(
